...
**Decisões pendentes de operação:** textos e visuais finais de cada card, orçamento e prazo. O papel do Labafero só será declarado quando estiver confirmado; a ausência dessa informação não impede um card descritivo.
...
**Proposed Solution:** Publicar uma landing page que funcione como showroom e portfólio: apresentar o Labafero, listar projetos com descrições breves, distinguir projetos e parcerias quando isso for relevante e permitir a exploração do trabalho. Usar Google Analytics 4 para observar visitas, origens e interesse pelos projetos, orientando o posicionamento futuro da marca.

**Success Criteria:**

- **Alcance:** registrar visualizações, sessões e usuários da landing page. Linha de base e meta numérica serão definidas após as primeiras quatro semanas de coleta.
- **Origem:** consultar referenciadores e parâmetros UTM para entender a contribuição de Instagram, LinkedIn e outras fontes.
- **Descoberta:** medir a proporção de sessões com `project_open` e a distribuição de aberturas por projeto. Meta numérica `TBD` após a linha de base inicial.
...
3. **Como responsável, quero observar o uso do showroom para orientar o posicionamento da marca.**
   - Em produção, GA4 registra visualizações de página, sessões, origens e eventos de exploração.
   - `projects_view`, `project_open` e `project_link` são enviados sem dados pessoais; `project_id` é um identificador estável do card.
   - O painel permite consultar evolução semanal e origens disponíveis. A dimensão personalizada `project_id` deve ser cadastrada no GA4 para relatórios por projeto.
...
**Architecture Overview:** aplicação Nuxt pública e responsiva, inicialmente com conteúdo local estruturado e sem CMS. A chamada principal leva à seção de projetos; cada card pode revelar um detalhe breve na própria página e oferecer um link externo opcional. O módulo `nuxt-gtag` integra o Google tag para visualizações e eventos em produção. A hospedagem é na Vercel, com `labafero.com` como domínio de produção.
...
- **Analytics:** Google Analytics 4 via `nuxt-gtag`, com ID `G-DQ10F88M3W`. Registrar `page_view`, `projects_view`, `project_open` e `project_link`; consultar origens e UTMs no GA4.
...
- **v1.1:** após quatro semanas de coleta, revisar visitas, origens do tráfego e exploração dos projetos; ajustar hierarquia, textos e ordem dos itens com base nos dados e no feedback recebido.
...
**Próximo passo:** validar a coleta no GA4 após o deploy de produção e usar os primeiros dados para revisar o posicionamento da marca.
